...
if __name__ == '__main__':
    ...
    red_data_aug = poly.fit_transform(red_data)
    white_data_aug = poly.fit_transform(white_data) 
    #regr = RandomForestRegressor(max_depth = 5, random_state = 0)d
    dro_model = Sq_Loss(red_data_aug, red_eval)
    for eps in [0, 0.01, 0.05, 0.1, 0.5, 1, 5, 10, 50, 100]:
        dro_model.standard_solver(robust_param = eps, reg = ['W1-2', 0], option = 'CVXPY')
        print(eps, dro_model.predict(red_data_aug, red_eval), dro_model.predict(white_data_aug, white_eval))
